...
function Connect-M365Services {
    ...
    try {
        ...
        $adminUrl = "https://$((Get-MgOrganization).VerifiedDomains | Where-Object IsDefault).Name.Replace('.onmicrosoft.com','')-admin.sharepoint.com"
...
function Get-ExternalUsers {
    ...
    try {
        ...
        # Method 1: Get Guest Users from Azure AD with pagination support
        $pageSize = 999
        $guestUsers = @()
        
        try {
            # Use pagination for large directories
            $guestUsers = Get-MgUser -Filter "userType eq 'Guest'" -All -PageSize $pageSize -Property @(
            ...
            Write-AuditLog "Error retrieving guest users from Azure AD: $($_.Exception.Message)" -Level Warning
            # Continue with empty collection
            $guestUsers = @()
        }
        
        foreach ($guest in $guestUsers) {
            # Robust null checking for SignInActivity
            $lastSignIn = $null
            ...
            $userObj = [PSCustomObject]@{
                ...
                Email = if ($guest.Mail) { $guest.Mail } elseif ($guest.UserPrincipalName) { $guest.UserPrincipalName } else { "N/A" }
                UPN = $guest.UserPrincipalName ?? "N/A"
                UserType = "Guest"
                CreatedDate = $guest.CreatedDateTime ?? (Get-Date).AddYears(-1)
                ExternalUserState = $guest.ExternalUserState ?? "Unknown"
                StateChangeDate = $guest.ExternalUserStateChangeDateTime
                CompanyName = $guest.CompanyName ?? "External"
                Department = $guest.Department
                JobTitle = $guest.JobTitle
                AccountEnabled = if ($null -eq $guest.AccountEnabled) { $true } else { $guest.AccountEnabled }
        ...
        # Method 2: Get External Users from SharePoint with pagination
        try {
            $spoExternalUsers = @()
            $position = 0
            $pageSize = 50
            ...
            do {
                $batch = Get-PnPExternalUser -PageSize $pageSize -Position $position -ErrorAction Stop
                if ($batch) {
                    $spoExternalUsers += $batch
                    $position += $pageSize
                } else {
                    break
                }
            ...
            Write-AuditLog "Error retrieving SharePoint external users: $($_.Exception.Message)" -Level Warning
            $spoExternalUsers = @()
        }
